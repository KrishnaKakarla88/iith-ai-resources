...
- **Default is `scheduled` (live-armed), not draft.** Every `create_post` call sets `saveToDraft: false` (or omits it) alongside `mode: "customScheduled"` and a real `dueAt` — the post WILL auto-publish to the live channel at that scheduled time, with no further review step. This was flipped from an earlier draft-only default once Krishna confirmed the end-to-end flow (image hosting, LinkedIn/Instagram field quirks, calendar visibility) worked correctly on a real test post. Only fall back to `saveToDraft: true` if Krishna explicitly asks for a specific post to be held as a draft instead.
- **`edit_post` re-validates the whole post from what you send, not a merge.** Whatever you pass (or omit) for `saveToDraft` on an edit is exactly what the post ends up as — omitting it does not preserve whatever status the post already had. Always pass `saveToDraft` explicitly (matching the status you intend: `false` to keep/make it scheduled, `true` to hold it as a draft) on **every** `edit_post` call, along with the full `text`/`assets`/`metadata` again (a bare status-only retry is rejected as "Post must have either text or media"). After any edit, check the returned `status` field matches what you intended before moving on.
